...
class REO(BaseMetric):
    ...
    def __user_pop_reo(self, user_recommendations, user_train, cutoff, user_relevant_items):
        ...
        for i, i_set in self._item_clustering.items():
            ...
            reos.append(len(recommended_items & i_set) / len((i_set  & user_relevant_items) - user_train) if len((i_set  & user_relevant_items) - user_train) != 0 else 0)
        reos.append(np.std(reos) / np.mean(reos) if np.mean(reos) != 0 else 0)
        return reos
    ...
    def process(self):
        ...
        """
        reos_user_dicts = []
        for i_category in range(self._item_n_clusters+1):
            reos_user_dicts.append({})
        for u, u_r in self._recommendations.items():
            if len(self._relevance.get_user_rel(u)):
                reos_user = self.__user_pop_reo(u_r, set(self._train[u].keys()), self._cutoff, set(self._relevance.get_user_rel(u)))
                for i_category in range(self._item_n_clusters+1):
                    reos_user_dicts[i_category][u] = reos_user[i_category]
        for i_category in range(self._item_n_clusters+1):
            if Settings.current_file != -1:
                save_dir = rf"C:\Users\fedea\OneDrive\Documentos\Rares\elliot\thesis_sources\evaluation\reo\{Settings.files[Settings.current_file]}"
                os.makedirs(save_dir, exist_ok=True)
                pd.Series(reos_user_dicts[i_category]).reset_index().to_csv(rf"{save_dir}\group_{i_category}_{self._cutoff}.tsv", sep="\t", index=False, header=["user", "value"])
        PR = self._num / self._den

        self._metric_objs_list = []
        reo_group = {}
        for i_category in range(self._item_n_clusters):
            reo_group[i_category] = PR[i_category]
        ...
        self._metric_objs_list.append(ProxyMetric(name=f"REO_items:{self._item_clustering_name}",
                                                  ...
                                                  needs_full_recommendations=False))
        if Settings.current_file != -1:
            save_dir = rf"C:\Users\fedea\OneDrive\Documentos\Rares\elliot\thesis_sources\evaluation\{Settings.files[Settings.current_file]}"
            os.makedirs(save_dir, exist_ok=True)
            pd.Series(reo_group).reset_index().to_csv(rf"{save_dir}\reo_{self._cutoff}.tsv", sep="\t", index=False, header=["group", "value"])
