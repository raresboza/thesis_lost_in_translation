...
class nDCG(BaseMetric):
    ...
    def eval_user_metric(self):
        ...
        """
        user_ndcg =  {u: self.__user_ndcg(u_r, u, self._cutoff)
        ...
        if Settings.current_file != -1:
            ...
            pd.Series(user_ndcg).reset_index().to_csv(rf"{save_dir}\ndcg_{self._cutoff}.tsv", sep="\t", index=False, header=["user", "value"])
        return user_ndcg
