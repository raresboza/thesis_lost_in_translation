...
class MAP(BaseMetric):
    ...
    def eval_user_metric(self):
        ...
        """
        user_map = {u: MAP.__user_ap(u_r, self._cutoff, self._relevance.get_user_rel(u))
        ...
        if Settings.current_file != -1:
            ...
            pd.Series(user_map).reset_index().to_csv(rf"{save_dir}\map_{self._cutoff}.tsv", sep="\t", index=False, header=["user", "value"])
        return user_map