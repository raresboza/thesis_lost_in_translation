...
class ProxyRecommender(RecMixin, BaseRecommenderModel):
    ...
    def get_single_recommendation(self, mask, k):
        print("in get_single_recommendation")
        ...
        print("get_single_recommendation: before list comprehension")
        for user, item in zip(*nonzero):
            candidate_items.setdefault(
                self._data.private_users[user], set()
            ).add(self._data.private_items[item])
        print("get_single_recommendation: after list comprehension")
        recs = {}
